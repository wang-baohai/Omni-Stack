...
/**
 * SRM 列表和详情负责人一次性批量展示增强器，Auth 不可用时保留 ID 降级。
 *
...
public class SrmOwnerEnricher {
     ...
     * @return 原列表
     */
    public <T extends SrmViews.OwnedVO> List<T> enrich(List<T> records) {
        ...
        try {
            ...
            log.warn("SRM 负责人展示增强降级为 ID：tenantId={}, message={}", tenantId, exception.getMessage());
        }
        return records;
    }
     ...
     * @return 原视图
     */
    public <T extends SrmViews.OwnedVO> T enrichOne(T record) {
