...
/**
 * SRM XSS 配置提供者，缓存未命中时回源 Auth，Auth 不可用时启用基线规则。
 *
...
public class XssConfigProviderImpl implements XssConfigProvider {
    ...
    public XssSettings getXssSettings(Long tenantId) {
        try {
            ...
            log.warn("SRM XSS 缓存读取或解析失败，将回源 Auth：tenantId={}, message={}", tenantId, exception.getMessage());
        }
        try {
            ...
            log.error("SRM XSS 配置回源失败，启用基线防护：tenantId={}, message={}",
    ...
    private void cacheAuthoritative(Long tenantId, XssSettings settings) {
        try {
            ...
            log.warn("SRM XSS 权威配置写入缓存失败：tenantId={}, message={}", tenantId, exception.getMessage());
        }
    }
