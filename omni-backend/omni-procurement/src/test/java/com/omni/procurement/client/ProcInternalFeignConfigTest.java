...
/** Procurement 内部 Feign 客户端统一认证头配置测试。 */
class ProcInternalFeignConfigTest {

    private static final String TOKEN = "procurement-feign-test-token-0123456789abcdef";

    /** Auth、SRM 与 Workflow 客户端都必须使用 Starter 工厂注入内部令牌。 */
    @Test
    void shouldUseStarterFactoryForEveryInternalClient() {
        ...
        List<RequestInterceptor> interceptors = List.of(
                ...
                new SrmInternalClient.FeignConfig().srmInternalTokenInterceptor(properties, factory),
                new WorkflowInternalClient.FeignConfig()
                        .workflowInternalTokenInterceptor(properties, factory));
