  ...
  >
    <div v-if="result">
      <el-result
        ...
        :title="result.valid ? t('workflow.validationPassed') : t('workflow.validationFailed')"
      />

      <div v-if="result.errors.length > 0" class="result-section">
        <h4 class="section-title error-title">{{ t('workflow.errorCount', { count: result.errors.length }) }}</h4>
      ...
      <div v-if="result.warnings.length > 0" class="result-section">
        <h4 class="section-title warning-title">{{ t('workflow.warningCount', { count: result.warnings.length }) }}</h4>
        <ul class="result-list">
