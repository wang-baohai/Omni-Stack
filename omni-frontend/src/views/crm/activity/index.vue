...
<template>
  <div class="page-container">
    ...
    <el-drawer v-model="detailVisible" title="活动详情" size="560px"><el-descriptions v-if="detailActivity" :column="1" border><el-descriptions-item label="主题">{{ detailActivity.subject }}</el-descriptions-item><el-descriptions-item label="类型 / 状态">{{ detailActivity.activityType }} / {{ detailActivity.status }}</el-descriptions-item><el-descriptions-item label="关联对象">{{ detailActivity.rootType }} #{{ detailActivity.rootId }}</el-descriptions-item><el-descriptions-item label="计划时间">{{ detailActivity.plannedStartTime || '-' }} 至 {{ detailActivity.plannedEndTime || '-' }}</el-descriptions-item><el-descriptions-item label="内容"><p class="plain-content">{{ detailActivity.content || '-' }}</p></el-descriptions-item><el-descriptions-item label="执行人">用户 #{{ detailActivity.performedByUserId || '-' }}</el-descriptions-item></el-descriptions></el-drawer>
  </div>
</template>
