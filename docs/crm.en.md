...
**Depends on**: `omni-common-service`, `omni-common-core`, `omni-common`, `omni-common-mybatis`, `omni-common-redis`, `omni-common-operlog`, `omni-common-job`, `omni-common-mqlog`.

**Do NOT depend on** `omni-common-workflow` — that would pull the Flowable engine into CRM.

**Cross-service calls**: Calls Auth internal APIs via OpenFeign + `X-Internal-Token` generated by `InternalFeignHeadersFactory`. CRM only stores userId/unitId and never reads `omni_auth` across databases.
...
→ @ServiceDataScope aspect → MyBatis DataPermission interceptor → CrmRecordAccessGuard row-level write auth
```

1. Gateway verifies RS256 JWT and injects `X-User-*`, `X-Tenant-Id`, `X-Gateway-Forwarded`
2. `GatewayPreAuthenticationFilter` of `omni-common-service` builds `Authentication`, and `ServiceIdentityFilter` validates and binds userId/tenantId
3. Controller `@PreAuthorize` checks functional permissions
4. The `@ServiceDataScope(permissionCode)` common aspect calls the Auth internal API to resolve dataScope
...
CRM provides domain policies to `omni-common-service` via `CrmTenantTablePolicy` and `CrmDataPermissionHandler`; the common Starter assembles `mybatisPlusInterceptor` with a fixed, non-negotiable order:
...
The `/` in the table is shorthand for multiple full permission codes under the same resource; each full code is stored individually. `@PreAuthorize` and `@ServiceDataScope` use the same full permission code.
...
4. **ThreadLocal Cleanup**: `ServiceIdentityContext` and `ServiceDataScopeContext` must be cleaned up in `finally` blocks to prevent memory leaks
5. **Dual Permission Declaration**: Write endpoints must declare both `@PreAuthorize` (functional permission) and `@ServiceDataScope` (data scope) with the same full permission code
...
5. Controller write endpoints declare `@PreAuthorize` + `@ServiceDataScope` with new `crm:<resource>:<action>` permission codes
...
3. Controller methods declare `@PreAuthorize("hasAuthority('crm:<resource>:<action>')")` + `@ServiceDataScope(permissionCode = "crm:<resource>:<action>")`
