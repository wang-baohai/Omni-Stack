...
- 전제 조건: `procurement:overview:view` 권한을 가진 조달 관리자로 로그인
- 작업자: 조달 관리자
...
입고 전표는 실수령 수량·품질 상태·자산 수량을 행별로 기록합니다. `qualityStatus=PASS`, 자재가 `assetManaged=true`, 자산 수량이 정확한 양의 정수일 때만 자산 후보 이벤트를 발행합니다. 중복 제출·메시지 재전달·Asset 백필은 멱등하게 유지됩니다.
