...
`core`、`workflow`、`crm`、`supply-chain`、`full` を選択できます。詳細は [プリセット早見表](../preset-quick-selection.jp.md)を参照してください。初回起動では Liquibase が先に移行を実行します。旧 `init-all.sql` や `migrate-*.sql` は実行しません。
...
#### 図 1 `system-dashboard-ja-JP`：コンソールダッシュボード

- 前提条件：管理者としてログイン
- 操作者：管理者
- 操作：ログイン後は承認ワークベンチに着地するため、右上の「コンソール」をクリックして管理ダッシュボードを開く
- 期待結果：メイン領域に「ようこそ」カードとバックエンド/フロントエンド/コアサービス/インフラ情報、サイドバーにシステム管理/マスターデータ/ジョブ管理/運用監視/ワークフロー管理が表示される

![コンソールダッシュボード](../images/ja-JP/system-dashboard.png)
