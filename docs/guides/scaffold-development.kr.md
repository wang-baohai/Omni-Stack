...
forward-only Liquibase changeSet, 안정 seed, `scripts/sql/seed/auth.sql` 권한, `database/seed/manifest.yaml`, fresh/upgrade 두 경로를 갱신합니다. `migrate-*.sql`이나 임시 복구 스크립트를 공식 산출물로 추가하지 않습니다.
