...
>
> **One command launches middleware, the database migrator, eight microservices, and the frontend: 16 Docker containers in full mode.**
...
- [Five-minute quick start](docs/guides/quick-start.en.md) is the shortest path to startup, login, and verification.
- The user guides continue from [authentication](docs/guides/authentication.en.md) through permissions, Workflow, Scheduling, CRM, SRM, Procurement, and Asset.
- Presets, `create-service`, CRUD generation, and lightweight mode are documented in the [scaffold development guide](docs/guides/scaffold-development.en.md); the maintained CLI lives in `tools/omni-cli`.
- Schema is managed by `omni-db-migrator` and `database/changelog/`; `scripts/sql/seed` contains idempotent seed data only. Back up before upgrade and use compatibility windows plus forward fixes for recovery.
- See [docs/observability.en.md](docs/observability.en.md) and the [operations and upgrade guide](docs/guides/operations-upgrade.en.md) for local telemetry and production security boundaries.
- The [English screenshot index](docs/images/en-US/auth-login.png) is generated by the documentation Playwright suite; module guides describe the actor, prerequisites, steps, and expected result.

Best suited to enterprise admin systems, tenant-aware data applications, and microservices that need Workflow, CRM, or supply-chain capabilities. It is not a no-code platform and does not replace domain modeling, production secret management, capacity planning, or human release approval.
...
- **On-demand development and one-click deployment**: Omni CLI starts the smallest core/workflow/crm/supply-chain/full dependency closure; `start.bat` / `./start.sh` defaults to the 16-container full mode — see [Docker Deployment Guide](docs/docker-deployment.en.md)
...
- **SRM → Procurement → Asset chain**: supplier onboarding and quotation, requisition/RFQ/order/receipt, and asset card/allocation/transfer/disposal cooperate as separate microservices.
...
- **Common Starter ecosystem**: 10 public modules including the Servlet composition starter `omni-common-service`; security capabilities fail closed when required context is missing.
...
├── compose.yaml                        # Unified Compose entry point
├── compose.infra.yaml / compose.apps.yaml
├── compose.observability.yaml          # Optional local observability stack
├── docker/
...
├── database/changelog/                 # Liquibase structure history and vendor schemas
├── scripts/sql/seed/                   # Idempotent seed data only
...
│   ├── omni-common-service/            #   Servlet business-service composition Starter
...
│   ├── omni-procurement/               #   Procurement execution service (8106)
│   ├── omni-asset/                     #   Asset lifecycle service (8107)
...
Full mode launches middleware, `omni-db-migrator`, eight backend microservices, and the frontend. Presets can start a smaller verified dependency closure.
...
| Node.js | >= 22.12.0 | Runs Omni CLI and startup scripts |
| Git | Any | To clone the project |

> Container startup does not require a local JDK or Maven; Omni CLI requires Node.js.

### Startup

| Platform | Command |
|----------|---------|
| Windows | `start.bat` (administrator privileges are not required) |
...
# Start the smallest verified CRM dependency closure
./start.sh crm

# Equivalent CLI, optionally with local observability
...
| Port binding failure | Another process or Compose project owns the port | Use `dev status` to locate and stop the conflict; administrator rights are not required |
...
```bash
# 1. Start one module and its minimum dependency closure (CRM example)
...
### Common Starter Ecosystem (10 Modules)
...
| `omni-common-service` | Servlet composition starter for common web, database, Redis, security, and observability integration | Business services |
...
| Docker port conflicts | Another process or Compose project owns the port | Use `dev status` to locate and stop the conflict; administrator rights are not required |
